		...
		return (
			<Group {...rest}>
				<Circle
					...
					onClick={e => { e.evt.preventDefault(); this.props.onClick(this.props.tensor, e.evt.offsetX, e.evt.offsetY) }}
				/>
				<Text
					text={'<' + tensor.shape.dims.join(', ') + '>'}
					x={isOutput ? (this.state.ref ? -this.state.ref.getTextWidth() + 2 : 0) : -2}
					y={isOutput ? -20 : 8}
