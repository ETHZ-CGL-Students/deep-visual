...
class MainHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        self.write(
            "<form method='GET'><select name='i' onchange='this.form.submit()'>"
        )
        ...
        for i in range(0, len(img)):
            self.write(
                "<rect x='{0}' y='{1}' width='10' height='10' style='fill:rgb({2:.0f},{2:.0f},{2:.0f})' />".
                format((i % 28) * 10,
                       math.floor(i / 28) * 10, round((1 - img[i]) * 255)))
        self.write("</svg></div>")
        ...
        self.write(
            "<table><thead><tr><th>Index</th><th>Value</th><th>Softmax</th></tr></thead><tbody>"
        )
        for i in range(0, len(res)):
            self.write(
                "<tr style='background-color:{0};'><td>{1}</td><td>{2: .10f}</td><td>{3: 1.10f}</td></tr>"
                .format(("green" if resPred == pred else "orange")
                        if resPred == i else
                        ("yellow" if resSoft[i] > resSoft[resPred] / 2 else
                         "transparent"), i, res[i], resSoft[i]))
        self.write("</tbody></table></div>")
...
class TestHandler(JsonHandler):
    def get(self, idx):
        ...
        for i in range(0, len(img)):
            obj["svg"] += "<rect x='{0}' y='{1}' width='10' height='10' style='fill:rgb({2:.0f},{2:.0f},{2:.0f})' />".format(
                (i % 28) * 10,
                math.floor(i / 28) * 10, round((1 - img[i]) * 255))
        obj["svg"] += "</svg>"
